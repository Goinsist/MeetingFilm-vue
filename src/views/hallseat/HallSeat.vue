...
<template>
  <div class="wapper">
    <!--头部 开始-->
    <header-view :title-text="movieName" @backHandleClick="back"/>
    <!--头部 结束-->
    <!--排期详情和座位上方示例图 开始-->
    <plan-detail :prop-hall-name="hallName" :prop-show-date="showDate" :prop-show-time="showTime">
      <template v-for="seatTypeItem in seatTypeList" >
        <div
          v-if="seatTypeItem.isShow==='1' && seatTypeItem.position==='up'"
          :key="'seatType'+seatTypeItem.type"
    ...
    <seat-area
      ref="seatArea"
      :prop-thumbnail-area-width="thumbnailBoxWidth"
      :prop-thumbnail-area-height="thumbnailBoxHeight"
      :prop-y-max="yMax"
      :prop-seat-scale="seatScale"
      :prop-seat-height="positionDistin"
      :prop-seat-tool-arr="seatToolArr"
      :prop-seat-area-width-rem="seatAreaWidthRem"
      :prop-seat-area-height-rem="seatAreaHeightRem"
      :prop-seat-box-height="seatBoxHeight"
      :prop-middle-line="middleLine"
      :prop-horizontal-line="horizontalLine"
    >
      <!--以下为缩略座位图具名插槽 开始-->
      <template slot="thumbnail-seat-solt">
        <template v-for="seatItem in seatList" >
          <div
            ...
            :style="{height:thumbnailHeight +'rem',
      ...
      <template slot="seat-area-solt">
        <div
          class="seatBox"
          :style="{transform: 'scale('+seatScale+')',height:seatBoxHeight +'rem',
                   width:seatBoxWidth +'rem',marginLeft:seatBoxCenterMargin+'rem'}"
        >
          <!--中轴线-->
          <div v-show="seatList.length>0" class="middle-line" :style="{height:seatBoxHeight +'rem',left: middleLine +'rem'}"/>
    ...
    <keep-alive>
      <component
        :is="selectedTabComponents"
        :prop-seat-list="seatList"
        :prop-selected-seat="selectedSeatList"
...
export default {
  ...
  methods: {
    ...
    getSeatList: function () {
      // mock数据
      this.$axios.get('/meetingFilm/cinema/getHallSeatsJson', {
        params: {
        ...
        .then((response) => {
          ...
          var resSeatList = response.data.seatList
          // 座位处理 -------开始
          if (!resSeatList) {
            return
          }

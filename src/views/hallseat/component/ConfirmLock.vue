...
<template>
  <div class="confirm" @click="lockSeat()">
    <div v-if="chPrice!==0"> {{ chPrice }}元 确认选座</div>
    <div v-if="chPrice===0">  确认选座</div>
  </div>
</template>
...
// import引入的组件需要注入到对象中才能使用
  ...
  data () {
    ...
    return {
      ...
      fieldId: String(this.$route.query.fieldId),
  ...
  created () {
    this.$axios.get('/meetingFilm/cinema/getShowTime', {
      params: {
  ...
  methods: {
    ...
    createOrder () {
      ...
      for (let i = 0; i < this.selectedSeat.length; i++) {
        soldSeats += this.selectedSeat[i].id + ','
        seatsName += '第' + this.selectedSeat[i].row + '排' + this.selectedSeat[i].col + '座' + ','
      }
      soldSeats = soldSeats.substring(0, soldSeats.length - 1)

      seatsName = seatsName.substring(0, seatsName.length - 1)
      let data = {
        fieldId: Number(this.$route.query.fieldId),
        soldSeats: soldSeats,
        seatsName: seatsName
      }
      console.log(qs.stringify(data))
      this.$axios
        .post('/meetingFilm/order/buyTickets', qs.stringify(data))
        .then(res => {
          if (res.status === 0) {
            console.log('订单详情')
            console.log(res)
            this.price = res.data.orderPrice
            this.orderTime = res.data.orderTimestamp
            this.$router.push({
              name: 'confirmOrder',
              query: {
